...
export default function DicomViewerSimple() {
    ...
    const getCurrentImageUrl = () => {
        if (!study || !study.series || study.series.length === 0) {
            console.warn('[DicomViewerSimple] No study or series');
            return null;
        }

        const currentSeries = study.series[activeSeries];
        if (!currentSeries) {
            console.warn('[DicomViewerSimple] No current series at index:', activeSeries);
            return null;
        }

        if (!currentSeries.instances || currentSeries.instances.length === 0) {
            console.warn('[DicomViewerSimple] No instances in series');
            return null;
        }

        const instance = currentSeries.instances[activeInstance];
        if (!instance) {
            console.warn('[DicomViewerSimple] No instance at index:', activeInstance);
            return null;
        }

        const sopUID = instance.sop_instance_uid || instance.sopInstanceUID || instance['00080018']?.Value?.[0];
        if (!sopUID) {
            console.error('[DicomViewerSimple] No SOP UID in instance:', instance);
            return null;
        }

        // Use WADO-RS /rendered endpoint for JPEG
        const imageUrl = wadoService.getRenderedUrl(study.id, currentSeries.seriesInstanceUID, sopUID);

        // Append auth token for Gateway authentication (required for <img> tags)
        const auth = getAuth();
    ...
    // Handlers
    const handleSeriesSelect = (index) => {
