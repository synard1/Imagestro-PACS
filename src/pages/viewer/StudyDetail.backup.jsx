...
export default function StudyDetail() {
  ...
  useEffect(() => {
    const loadStudy = async () => {
      console.log('[StudyDetail] Loading study:', studyUID);
      ...
      if (serviceStudy) {
        console.log(`[StudyDetail] Study loaded from ${source}:`, studyUID);
        ...
        if ((source === 'backend' || source === 'wado-fallback') && (!seriesData || seriesData.length === 0)) {
          try {
            ...
            console.log('[StudyDetail] Fetching series separately for:', studyInstanceUID);
            ...
            if (series && series.length > 0) {
              console.log('[StudyDetail] Fetched separate series:', series.length);
              seriesData = series;
            }
          } catch (err) {
        ...
        if (source === 'localStorage') {
          try {
            ...
            console.log('[StudyDetail] Looking for series with studyUID:', studyUIDForSeries);

            const localSeries = getSeriesByStudyUID(studyUIDForSeries);
            console.log('[StudyDetail] Found series from localStorage:', localSeries.length);
            ...
            console.log('[StudyDetail] Loaded series from localStorage:', seriesData.length);
          } catch (error) {
        ...
        foundStudy = {
          ...
          patientId: serviceStudy.patient_id || serviceStudy.patientId || 'Unknown',
          patientDOB: serviceStudy.patient_dob || serviceStudy.patientDOB || '',
          ...
          referringPhysician: serviceStudy.referring_physician || 'Dr. Unknown',
        ...
        console.log('[StudyDetail] Normalized study:', foundStudy);
      } else {
        console.log('[StudyDetail] Study not found in service, trying JSON files');
      ...
      if (foundStudy) {
        console.log('[StudyDetail] Study found and set:', foundStudy);
        setStudy(foundStudy);
      } else {
  ...
  const handleOpenViewer = () => {
    // Navigate to enhanced viewer with study ID
    ...
    try {
      ...
      if (series && series.length > 0) {
        ...
        if (instances && instances.length > 0) {
          ...
          console.log('[StudyDetail] Opening tag viewer for instance:', firstInstance.id);
              ...
              {study.series.map((series, idx) => (
                ...
                >
                  ...
                  <div className="aspect-square bg-gray-900 rounded-lg mb-3 flex items-center justify-center">
                    <div className="text-white text-4xl opacity-20">{study.modality}</div>
                  </div>
