...
export async function getIncomingOrders(filters = {}) {
  try {
    ...
    // Filter by intake-relevant statuses
    const intakeStatuses = ['created', 'scheduled', 'arrived'];
    if (filters.status) {
      params.append('status', filters.status);
    } else {
      // Default: show orders that need intake processing
      intakeStatuses.forEach(status => params.append('status', status));
    }
    ...
    console.log('[intakeService] Loaded orders from backend:', orders.length);
    return orders;
  } catch (error) {
    ...
    try {
      ...
      return allOrders.filter(o => 
        ['created', 'scheduled', 'arrived'].includes(o.status)
      );
    } catch (mockError) {
