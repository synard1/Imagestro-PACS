...
const getWadoBaseUrl = () => {
  ...
  // Use V2 endpoint as per S3 integration guide
  return `${cleanBaseUrl}/wado-rs/v2`;
};
...
export const wadoService = {
  ...
  async getStudy(studyId) {
    try {
      ...
      console.log(`[WadoService] Fetching study from: ${baseUrl}/studies/${studyId}`);
      ...
      console.log(`[WadoService] Received WADO data:`, data ? (Array.isArray(data) ? `Array(${data.length})` : 'Object') : 'null');
      return data;
    } catch (error) {
  ...
  async downloadInstanceWithCache(studyId, seriesId, instanceId) {
    try {
      ...
      if (cached) {
        console.log(`[WadoService] Loading from cache: ${instanceId}`);
      ...
      console.log(`[WadoService] Downloading from backend: ${instanceId}`);
      const url = this.getInstanceUrl(studyId, seriesId, instanceId);
      const response = await fetch(url);
      ...
      try {
        ...
        console.log(`[WadoService] Cached image: ${instanceId}`);
      } catch (cacheError) {
