...
export const listOrders = async (params = {}) => {
  ...
  const cacheKey = `listOrders:${JSON.stringify(params)}`;
  
  // Return existing promise if request is already in progress
  if (_pendingRequests.has(cacheKey)) {
    logger.debug('[orderService] Deduplicating concurrent listOrders request');
    return _pendingRequests.get(cacheKey);
  }

  const listOrdersPromise = (async () => {
  ...
  _pendingRequests.set(cacheKey, listOrdersPromise);

  try {
    return await listOrdersPromise;
  } finally {
    // Remove from map when finished so subsequent requests can fetch fresh data
 ...
 * @returns {Promise<Object>} Updated order data
 */
export const updateOrderStatus = async (identifier, status, notes = '') => {
  logger.debug('[orderService] Updating order status:', identifier, 'to', status);
  
  // Build the update payload with status and optional notes
  const updateData = {
    status: status,
    order_status: status
  };
  ...
  // Use the existing updateOrder function
  return updateOrder(identifier, updateData);
};
