...
import { logSatusehatHttp } from "./db";
...
async function performTokenFetch(): Promise<string> {
  ...
  try {
    ...
    console.log(`[satusehat] Token cached, expires in ${expiresIn}s, will refresh in ${expiresIn - bufferSeconds}s`);
...
export const Satusehat = {
  ...
  searchEncounterBySubject: async (subject: string) => {
    const base = urls.fhir("Encounter");
    return fhirGet(base, { subject });
  },
  ...
  createServiceRequest: async (serviceRequestData: {
    ...
    note?: string;
    performerId?: string; // Add optional performer ID
  }) => {
  ...
  createEncounterMinimal: async (patientId: string, opts?: { classCode?: string; status?: string; locationRef?: string; serviceType?: string }) => {
    ...
    if (opts?.serviceType) {
      body.serviceType = {
        coding: [{ system: "http://terminology.hl7.org/CodeSystem/service-type", code: opts.serviceType }],
      };
    }
    const base = urls.fhir("Encounter");
    const resp = await fhirPost(base, body);
