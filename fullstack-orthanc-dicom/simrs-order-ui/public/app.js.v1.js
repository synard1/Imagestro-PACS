...
async function generateEncounterId() {
  ...
  if (!practitionerNik && !practitionerId) {
    toast("Practitioner NIK atau ID harus diisi untuk membuat encounter", "error");
    return;
  }
    ...
    if (practitionerNik) {
      ...
      if (pRes.ok) {
        ...
        if (id) {
          ...
          toast("Practitioner dengan NIK " + practitionerNik + " tidak ditemukan", "error");
          return;
        }
      } else {
        toast("Gagal mencari practitioner dengan NIK " + practitionerNik, "error");
        return;
      }
      ...
      if (isIhsNumber) {
        ...
        if (pRes.ok) {
          ...
          if (id) {
            ...
            toast("Practitioner dengan IHS Number " + practitionerId + " tidak ditemukan", "error");
            return;
          }
        } else {
          toast("Gagal mencari practitioner dengan IHS Number " + practitionerId, "error");
          return;
        }
      } else {
...
async function searchEncounterById() {
  try {
    ...
    const base = gatewayApiBase();
    const resp = await fetch(`${base}/satusehat/encounter/patient/${patientId}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${state.token}`,
    ...
    if (data.entry.length === 1) {
      ...
      const encIdField = qs("#satusehat_encounter_id");
      if (encIdField && confirm(`Ditemukan 1 encounter. Gunakan ID: ${firstEncounterId}?`)) {
