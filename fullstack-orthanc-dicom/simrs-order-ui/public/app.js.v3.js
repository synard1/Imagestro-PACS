...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    window.Utils?.toast?.("Gagal menginisialisasi aplikasi: " + (e?.message || e), "error");
  }
});
...
async function initializeApplication() {
  ...
  const { initializeUI, populateLocationDropdown, updateCopyButtonStatus } = window.UI;
  ...
  try {
    const cached = JSON.parse(localStorage.getItem("satusehat_locations") || "[]");
...
function setupEventListeners() {
  ...
  const { generateEncounterId, searchEncounterById, resolveSatusehatPatientId } = window.API;
  ...
  if (btnResolvePatient) btnResolvePatient.addEventListener("click", resolveSatusehatPatientId);

  // Copy encounter ID
  const btnCopyEncounter = qs("#copy-encounter-btn");
  if (btnCopyEncounter) btnCopyEncounter.addEventListener("click", copyEncounterId);
...
function setupFormFieldListeners() {
  ...
  if (regDateEl) {
    regDateEl.addEventListener("change", autoGenerateRegistrationOnScheduleChange);
    regDateEl.addEventListener("input", autoGenerateRegistrationOnScheduleChange);
  ...
  if (schedEl) {
    schedEl.addEventListener("change", autoGenerateRegistrationOnScheduleChange);
...
function applyDummyFromPreset(e) {
  const { qs, SAMPLE_PATIENTS, SAMPLE_PRACTITIONERS, PROCEDURE_SAMPLES } = window.Utils;
  const { fillPatientData, fillPractitionerData, fillProcedureData } = window.UI;
...
function doPractitionerSearch() {
  ...
  const filtered = SAMPLE_PRACTITIONERS.filter(p => 
    p.name.toLowerCase().includes(query) ||
