...
DB_HOST = 'postgres'
DB_PORT = 5432
DB_PASSWORD = os.getenv('ORTHANC_PASSWORD', 'orthanc123')
...
def apply_schema():
    ...
    try:
        ...
        # 0) Migration Block
        # =========================
        logger.info("Checking for legacy 'orgs' table to migrate...")
        cursor.execute("""
            DO $$
            BEGIN
                IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'orgs')
                AND NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'satusehat_orgs') THEN
                    ALTER TABLE orgs RENAME TO satusehat_orgs;
                    logger.info("Renamed table 'orgs' to 'satusehat_orgs'.");
                END IF;
            END $$;
        """)
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS orders (
                ...
                station_ae_title text NOT NULL,
                scheduled_start_at timestamptz,
                status text NOT NULL DEFAULT 'scheduled',
                created_at timestamptz NOT NULL DEFAULT now(),
                updated_at timestamptz NOT NULL DEFAULT now(),
                UNIQUE(org_id, accession_no),
                
                -- Enhanced patient information fields (aligning with patient table)
                ...
                -- Audit metadata (actors + timestamps)
        ...
        cursor.execute("""
            ...
            SELECT o.id AS order_id, o.accession_no, o.modality, o.patient_name,
        ...
        cursor.execute("""
            WITH o AS (SELECT id FROM satusehat_orgs LIMIT 1)
            INSERT INTO orders (id, org_id, patient_id, patient_name, accession_no, modality, requested_procedure, station_ae_title, scheduled_start_at, status)
            SELECT gen_random_uuid(), id, 'MRN001', 'Budi Santoso', '251028-0042', 'CT', 'CT Head Non-Contrast', 'CT_ROOM1', now(), 'completed' FROM o
            ON CONFLICT DO NOTHING
        """)
