...
class StorageMonitorService:
    ...
    def get_storage_by_modality(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        try:
            # First get basic counts from database
            results = self.db.query(
                ...
                Study.series,  # Join Study -> Series
            ).outerjoin(
                Series.instances,  # Join Series -> Instance
            ).filter(
                Study.deleted_at.is_(None),
                Study.modality.isnot(None)
            ).group_by(Study.modality).all()
            
            # Calculate actual file sizes from file system if database fields are empty
            modality_stats = {}
                ...
                for path_result in instance_paths:
                    ...
                    if file_path:
                        ...
                        if not file_path.startswith('/'):
                            file_path = f"{storage_path}/{file_path}"
