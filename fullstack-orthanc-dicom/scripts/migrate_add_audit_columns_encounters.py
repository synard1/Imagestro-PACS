#!/usr/bin/env python3
"""
Migration script: Add audit trail columns to satusehat_encounters table
...
def get_db_connection():
    ...
    password = os.getenv("DB_PASSWORD", os.getenv("POSTGRES_PASSWORD", "E4RMTJiyTmfUwk+tztrRKw=="))
...
def migrate_add_audit_columns():
    """Add audit trail columns to satusehat_encounters table"""
    conn = get_db_connection()
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            print("Starting migration: Add audit trail columns to satusehat_encounters")
            ...
            cursor.execute("""
                SELECT EXISTS (
                    ...
                    WHERE table_name = 'satusehat_encounters'
                )
            """)
            if not cursor.fetchone()['exists']:
                print("ERROR: Table satusehat_encounters does not exist!")
                return False

            # Add created_by column
            if not column_exists(cursor, 'satusehat_encounters', 'created_by'):
                print("Adding column: created_by")
                cursor.execute("""
                    ALTER TABLE satusehat_encounters
            ...
            if not column_exists(cursor, 'satusehat_encounters', 'updated_by'):
            ...
            if not column_exists(cursor, 'satusehat_encounters', 'deleted_at'):
            ...
            if not column_exists(cursor, 'satusehat_encounters', 'deleted_by'):
            ...
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_satusehat_encounters_created_by
                ON satusehat_encounters(created_by)
            """)
            print("✓ Index on created_by created")

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_satusehat_encounters_updated_by
                ON satusehat_encounters(updated_by)
            """)
            print("✓ Index on updated_by created")

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_satusehat_encounters_deleted_at
                ON satusehat_encounters(deleted_at)
