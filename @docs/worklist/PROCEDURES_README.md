...
- Menu: `Master Data` → `Procedures`
- Halaman: `Procedures` (list/filter), `Add Procedure`, `Edit Procedure`
- Field utama UI: `code`, `name`, `description`, `category`, `duration_minutes`, `special_requirements`
- Validasi: `code` unik dan format alfanumerik, `name` wajib, `category` wajib, `duration_minutes` angka ≥ 0
- Audit trail: perubahan tercatat di backend (`procedure_audit_log`) dan ditampilkan melalui log service bila tersedia
...
1. Admin membuka menu `Procedures`
2. Melakukan pencarian atau filter berdasarkan `code`, `name`, `category`, `modality`
3. Menambahkan prosedur baru melalui `Add Procedure`
4. Mengedit prosedur yang ada melalui aksi `Edit`
5. Menghapus prosedur (soft delete) bila tidak dipakai
6. Sistem menyimpan audit trail setiap perubahan
...
- Buat prosedur baru: Klik `Master Data` → `Procedures` → `Add Procedure`, isi field minimal (`code`, `name`, `category`), simpan.
- Edit prosedur: Buka `Procedures`, pilih `Edit` pada baris terkait, ubah informasi, simpan.
- Hapus prosedur: Klik `Delete` pada baris prosedur (akan melakukan soft delete).
- Filter: Gunakan kotak pencarian dan filter `category`/`modality` di bagian atas tabel.
...
UI memudahkan validasi dan pemilihan prosedur saat pembuatan order, termasuk menampilkan durasi dan instruksi persiapan.
...
Tambahkan instruksi persiapan yang relevan dan pastikan durasi estimasi realistis untuk kelancaran workflow radiologi.
...
### Issue: Validation error saat create/update
**Solution:** Pastikan `code`, `name`, `category` terisi; `duration_minutes` numerik ≥ 0; format `code` hanya alfanumerik, dash, dot, underscore.
