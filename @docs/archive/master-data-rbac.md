...
- The UI calls patient/doctor/settings endpoints directly on Master Data Service via the API Gateway base URL; authorization is enforced using the JWT issued by Auth Service.
- User management traffic is not handled here; it now goes to gateway `/auth/*` and then to the Auth Service.
- Ensure Gateway and Master Data share JWT secret/algorithm so the UI token is valid end-to-end.
