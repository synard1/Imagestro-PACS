...
- UI now fetches user/role/permission data via API Gateway.
- Registry defaults updated: `users.enabled = true`, `users.baseUrl = http://103.42.117.19:8888`.
- All user management calls use gateway routes under `/auth/*` and include Authorization Bearer from login.
- Expected payloads:
  - List users: `GET /auth/users?page=1&limit=20&search=&role=` returns `{ data: { users: [...], pagination: { page, limit, total, total_pages }}}`.
  - Roles: `GET /auth/roles` returns `{ data: { roles: [...] } }`.
  - Permissions: `GET /auth/permissions` returns `{ data: { permissions: [...] } }`.

Frontend service mapping:
- `src/services/userService.js` uses `apiClient('users')` with gateway base URL and calls the `/auth/*` endpoints below.
- Toggle via Settings → Integration → API Registry if needed.
