...
- Menu: `Master Data` → `Procedure Mappings`
- Halaman: `Procedure Mappings` (list/filter/export/import), `Add Mapping`, `Edit Mapping`
- Fitur UI:
  - Filter berdasarkan `external_system`, `external_code`, dan `mapping_type`
  - Ekspor JSON: unduh berkas `procedure-mappings.json` dari tampilan saat ini
  - Impor JSON: unggah berkas berformat `{ "mappings": [...] }` untuk bulk import
  - Backup/Restore: gunakan ekspor sebagai backup dan impor untuk restore
...
- Backup: Klik `Export JSON` pada halaman `Procedure Mappings` untuk mengunduh semua mapping yang difilter
- Restore: Klik `Import JSON`, pilih berkas hasil ekspor sebelumnya, sistem akan melakukan bulk import
- Format berkas:
```json
{
  "mappings": [
...
- Gunakan `Export JSON` dari lingkungan lama, kemudian `Import JSON` di lingkungan baru
- Pastikan `external_system_id` dan `pacs_procedure_id` valid di lingkungan target sebelum import
